export default function Card3() {
    return (
        <section className="flex justify-center items-center">
            <div className="bg-blue-700 rounded-3xl p-5 m-5 overflow-hidden text-white text-center flex flex-col justify-between select-none hover:scale-105 transition duration-500" >
                <h1 className="text-3xl m-5 font-bold" >Nosso plano que mais agrega benefícios!</h1>

                <p className="text-xl my-3" >Acesso às nossas instalações 5 vezes por semana</p>

                <p className="text-xl my-3" >Acesso aos vestuários e chuveiros</p>

                <p className="text-xl my-3" >Agendamento de avaliação física</p>

                <p className="text-xl my-3" >Aulas em grupo</p>

                <p className="text-xl my-3" >Eventos especiais e Workshops</p>

                <h3 className="text-5xl m-5 font-bold" >R$ 150,00</h3>
            </div>
        </section>
    )
}
...
export default function PagamentoLista() {
    ...
    const fetchPagamentos = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/api/pagamento/mostrarpagamentosreverso`);
            ...
            if (Array.isArray(pagamentosData)) {
                const pagamentosConvertidos = await Promise.all(
                    pagamentosData.map(async (pagamento) => {
                        const clienteResponse = await axios.get(`https://pi-academia.vercel.app/api/cliente/findbyid/${pagamento.clienteID}`);
